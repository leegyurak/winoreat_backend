...
class ListRestaurantSerializer(serializers.ModelSerializer):
    ...
    def get_reviews(self, obj) -> list[str]:
        if hasattr(obj, "review_posts"):
            return [
                review.post 
                for review in Review.objects.select_related('restaurant').filter(restaurant__name=obj.name)]
        return []
    
    def get_images(self, obj) -> list[str]:
        return [
            restaurant_image.img_url
            for restaurant_image in RestaurantImage.objects.select_related('restaurant').filter(restaurant__name=obj.name).order_by('-created')[:2]
        ]
